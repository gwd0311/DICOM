...
///
/// \brief A pipe can be used to push and pull data to/from an DICOMHERO codec.
///
/// This is useful when an DICOMHERO codec must be used with a data source
/// or a data sink not supported by the library (e.g. a TLS stream).
///
/// In order to allow DICOMHERO to read data from a custom data source:
/// - allocate a DicomheroPipe class and use it as parameter for the
...
///
/// In order to allow DICOMHERO to write data to a custom data source:
/// - allocate a DicomheroPipe class and use it as parameter for the
